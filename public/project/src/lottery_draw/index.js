$(function () {
    let num = 0; //初始化的选择的奖品索引
    let speedNum = 0; //转动了次数
    let time = null; //创建的setInterval
    let speed = 1000; //抽奖选择的速度
    let isClick = true; //
    let isDraw = false; //是否抽过奖
    let NoticeData = [{
            text: '158*******9获得一个月VIP！'
        },
        {
    ...
    $('.draw-active').on('mousedown',function() {
        $(this).attr('src','../../images/lottery_draw/choujiang@3x.gif')
    })
    // $('.draw-active').on('mouseup',function() {
    //     $(this).attr('src','../../images/lottery_draw/choujiang@3x.png')
    // })
    ...
    function drawActive() {
        ...
        if (speedNum > 56) {
            ...
            if (num === 2 || num === 4 || num === 7) {
                ...
                $('.draw-active').attr('src','../../images/lottery_draw/choujiang@3x.png')
                isClick = true;
            }
        } 
        $('.kinds-c').eq(num).addClass('active').parent().siblings().children().removeClass('active')
    ...
    function getCountDown(times) {
        let set = setInterval(() => {
            ...
            let [day, hour, minute, seconde] = [null, null, null, null]
    ...
    function createNoticeList(data) {
        ...
        let noticeNum = 0;
        let noticeSet = setInterval(function () {
            noticeNum++;
